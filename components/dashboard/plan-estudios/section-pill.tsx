...
function SectionPillInner( {
	...
	// O si el estado de red de los cupos en tiempo real presenta fallas (error / timeout)
	const isDisabled = !isPeriodActive || isPending || status === 'error' || status === 'timeout' || ( mode === 'toma_ramos'
		...
		<>
			<button
				...
				className       = { [
					...
					status === 'timeout'
						? 'bg-muted/10 border-muted-foreground/30 animate-pulse text-muted-foreground/70'
						: status === 'error'
				...
				<div className="flex items-center gap-2">
					...
					<div className="grid">
						...
						<span className="text-[9px] text-muted-foreground">
							{ status === 'timeout' ? (
								<span className="animate-pulse">Cargando cupos...</span>
							) : status === 'error' ? (
								<span className="text-red-500 font-semibold">Espera mientras se restablece la conexión</span>
							) : (
								section.professor
							) }
						</span>
